<form id="inputForm" name="inputForm" class="form-horizontal" action="./preview.php?preview=1" method="post" enctype="multipart/form-data">
	<div class="ibox-content">
		...
		<div class="form-group">
			<label class="col-sm-2 control-label">間取り</label>
			<div class="col-sm-6">
				{if $message.ng.layout|default:"" != NULL}<p class="error">{$message.ng.layout}</p>{/if}
				<input type="text" class="form-control" name="layout" id="layout" value="{$arr_post.layout|default:""}" />
			</div>
		</div>
		<div class="hr-line-dashed"></div>
		<div class="form-group">
			<label class="col-sm-2 control-label">作業人数</label>
			<div class="col-sm-6">
				{if $message.ng.person|default:"" != NULL}<p class="error">{$message.ng.person}</p>{/if}
				<input type="text" class="form-control" name="person" id="person" value="{$arr_post.person|default:""}" />
			</div>
		</div>
		<div class="hr-line-dashed"></div>
		<div class="form-group">
			<label class="col-sm-2 control-label">作業時間</label>
			<div class="col-sm-6">
				{if $message.ng.time|default:"" != NULL}<p class="error">{$message.ng.time}</p>{/if}
				<input type="text" class="form-control" name="time" id="time" value="{$arr_post.time|default:""}" />
			</div>
		</div>
		<div class="hr-line-dashed"></div>
		<div class="form-group">
			<label class="col-sm-2 control-label">作業料金</label>
			<div class="col-sm-6">
				{if $message.ng.price|default:"" != NULL}<p class="error">{$message.ng.price}</p>{/if}
				<input type="text" class="form-control" name="price" id="price" value="{$arr_post.price|default:""}" />
			</div>
		</div>
		<div class="hr-line-dashed"></div>
		{if $_ARR_IMAGE != NULL}
