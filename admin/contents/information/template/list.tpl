...
<table class="footable table table-stripped toggle-arrow-tiny tbl_1" data-page-size="15">
	...
	<tbody>
		...
		<tr>
			...
			<td class="pos_al">{$OptionInfoCategory[$data.id_category]}</td>
			<td><a href="./edit.php?id={$data.$_CONTENTS_ID}">{$data.title}</a></td>
			<td class="pos_al">
				<div class="lightBoxGallery">
					{foreach from=$_ARR_IMAGE item=file name=file}
						{if $data[$file.name]}
							<a href="{$_IMAGEFULLPATH}/{$_CONTENTS_DIR}/{$file.name}/l_{$data[$file.name]}" title="{$file.comment|default:""}" class="lightbox" rel="lightbox[]">
