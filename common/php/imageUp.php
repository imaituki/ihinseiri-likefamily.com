	...
	if( file_exists( $config ) == true ) {
		...
		if( is_array( $_ARR_IMAGE ) ) {
			...
			foreach( $_ARR_IMAGE as $key => $val ) {
				...
				if( !empty( $_FILES[$val["name"]]["tmp_name"] ) ) {
					...
					if( $originImage["extension"] < 1 || 3 < $originImage["extension"] ) {
						...
						// 画像をセッションに保存
						unset( $_SESSION["front"][$arr_post["_contents_dir"]]["preview"][$val["name"]] );
						$_SESSION["front"][$arr_post["_contents_dir"]]["preview"][$val["name"]]["type"]     = $_FILES[$val["name"]]["type"];
						$_SESSION["front"][$arr_post["_contents_dir"]]["preview"][$val["name"]]["tmp_name"] = $pathInfo["dirname"] . "/s_" . $pathInfo["filename"] . $ext;
						$_SESSION["front"][$arr_post["_contents_dir"]]["preview"][$val["name"]]["fn"]       = $fn;
